...
int main() {
	Mat img_color = imread("input.PNG", IMREAD_COLOR);
	Mat img_original;
	...
	for (int original_y = 0; original_y < original_h - template_h; original_y++) { // data로 봅음녀 1차원 어레이형태이다. 
		uchar * data_original = img_original.data; 
		for (int original_x = 0; original_x < original_w - template_w; original_x++) {
			int SAD = 0; //템플릿 이미지 스캔
